...
public class Printer3DDTO extends MakerToolDTO {
    ...
    private final Integer layerThickness;
    private final Printer3DType type;

    // Constructors.

    public Printer3DDTO(Integer id, String name, String description, String reference, List<MaterialDTO> materials, Integer x, Integer y, Integer z,
                        Integer xAccuracy, Integer yAccuracy, Integer zAccuracy, Integer layerThickness, Printer3DType type) {
        super(id, name, description, reference, materials);
        this.x = x;
        this.y = y;
        this.z = z;
    ...
    public Printer3DDTO(Printer3D printer3D) {
        this(printer3D.getId(), printer3D.getName(), printer3D.getDescription(), printer3D.getReference(),
             printer3D.getMaterials().stream().map(Material::toDTO).toList(), printer3D.getX(), printer3D.getY(), printer3D.getZ(),
             printer3D.getXAccuracy(), printer3D.getYAccuracy(), printer3D.getZAccuracy(),
             printer3D.getLayerThickness(), printer3D.getType());
    }
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equal(x, that.x) && Objects.equal(y, that.y) &&
                ...
                Objects.equal(layerThickness, that.layerThickness) && type == that.type;
    }
